...
{
    if (!is_open())
        SGL_THROW("Attempted to write to a closed memory stream");

    if (!is_writable())
        SGL_THROW("Attempted to write to a read-only memory stream");
...
{
    if (!m_owns_data)
        SGL_THROW("Cannot resize non-owned buffer");
