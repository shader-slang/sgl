...
def test_load_module(device_type):
    ...
    with pytest.raises(
        kali.SlangCompileError, match="unexpected end of file, expected identifier"
    ):
...
def test_load_module_from_source(device_type):
    ...
    # Loading a valid module must succeed
    module = device.load_module_from_source(module_name="module_from_source", source="""
        struct Foo {
            uint a;
        };
    ...
    assert main.stage == kali.ShaderStage.compute
