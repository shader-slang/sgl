...
template<typename T>
struct fmt::formatter<kali::print_buffer::Value<T>> : formatter<T> {
    template<typename FormatContext>
    auto format(const kali::print_buffer::Value<T>& v, FormatContext& ctx) const
    {
        auto out = ctx.out();
        switch (v.layout.kind) {
        case kali::print_buffer::Kind::scalar:
            out = formatter<T>::format(v.elements[0], ctx);
            break;
        case kali::print_buffer::Kind::vector:
            ...
            break;
        case kali::print_buffer::Kind::matrix:
...
namespace print_buffer {
    ...
    inline Value<T> decode_value(std::span<const uint8_t> data, const Layout layout)
    {
        ...
        case Kind::matrix:
            ...
            break;
        default:
            KALI_THROW("Invalid argument kind in print()");
        }

        KALI_ASSERT(value.element_count <= Value<T>::MAX_ELEMENT_COUNT);

        // Elements are aligned to 4 bytes.
        uint32_t element_size = ((uint32_t(sizeof(T)) + 3) / 4) * 4;
        KALI_ASSERT(data.size() >= element_size * value.element_count);
    ...
    inline void decode_arg(std::span<const uint8_t> data, fmt::dynamic_format_arg_store<fmt::format_context>& arg_store)
    {
        ...
        case Type::float64:
            ...
            break;
        default:
            KALI_THROW("Invalid argument type in print()");
        }
    }
    ...
    decode_msg(std::span<const uint8_t> data, const std::map<uint32_t, std::string>& hashed_strings, Output output)
    {
        ...
        KALI_ASSERT(data.size() >= 12);
        uint32_t msg_size = *reinterpret_cast<const uint32_t*>(ptr);
        KALI_ASSERT(data.size() == msg_size);
        ...
        try {
            ...
            KALI_THROW("Invalid shader print() formatting: {}\nFormat string:\n{}", e.what(), fmt);
        }
    }
