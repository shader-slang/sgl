...
template<typename T>
struct fmt::formatter<kali::ref<T>> : formatter<const void*> {
    template<typename FormatContext>
    auto format(const kali::ref<T>& ref, FormatContext& ctx) const
    {
...
template<typename T>
struct fmt::formatter<kali::breakable_ref<T>> : formatter<const void*> {
    template<typename FormatContext>
    auto format(const kali::breakable_ref<T>& ref, FormatContext& ctx) const
    {
        return formatter<const void*>::format(ref.get(), ctx);
    }
};
