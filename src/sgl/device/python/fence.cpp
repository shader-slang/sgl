...
SGL_PY_EXPORT(device_fence)
{
    ...
    nb::class_<Fence, DeviceResource>(m, "Fence", D(Fence))
        ...
        .def("signal_device", &Fence::signal_device, "value"_a = Fence::AUTO, "queue"_a = CommandQueueType::graphics, D_NA(Fence, signal_device))
        .def("wait", &Fence::wait, "value"_a = Fence::AUTO, "timeout_ns"_a = Fence::TIMEOUT_INFINITE, D(Fence, wait))
        .def("wait_device", &Fence::wait_device, "value"_a = Fence::AUTO, "queue"_a = CommandQueueType::graphics, D_NA(Fence, wait_device))
