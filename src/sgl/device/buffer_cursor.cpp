...
std::string BufferCursor::to_string() const
{
    return "BufferCursor()";
}

BufferCursor BufferCursor::operator[](std::string_view name) const
{
    SGL_CHECK(is_valid(), "Invalid cursor");
    BufferCursor result = find_field(name);
    SGL_CHECK(result.is_valid(), "Field \"{}\" not found.", name);
    return result;
}

BufferCursor BufferCursor::operator[](uint32_t index) const
{
    SGL_CHECK(is_valid(), "Invalid cursor");
    BufferCursor result = find_element(index);
    SGL_CHECK(result.is_valid(), "Element {} not found.", index);
    return result;
}

BufferCursor BufferCursor::find_field(std::string_view name) const
{
    if (!is_valid())
        return *this;
...
BufferCursor BufferCursor::find_element(uint32_t index) const
{
    if (!is_valid())
        return *this;
...
void BufferCursor::set_data(const void* data, size_t size) const
{
...
void BufferCursor::_set_scalar(const void* data, size_t size, TypeReflection::ScalarType scalar_type) const
{
    check_scalar(size, scalar_type);
    write_data(m_offset, data, size);
}

void BufferCursor::_get_scalar(void* data, size_t size, TypeReflection::ScalarType scalar_type) const
{
    check_scalar(size, scalar_type);
    read_data(m_offset, data, size);
}

void BufferCursor::_set_vector(const void* data, size_t size, TypeReflection::ScalarType scalar_type, int dimension)
    const
{
...
void BufferCursor::_get_vector(void* data, size_t size, TypeReflection::ScalarType scalar_type, int dimension) const
{
    check_vector(size, scalar_type, dimension);
    read_data(m_offset, data, size);
}

void BufferCursor::_set_matrix(
    const void* data,
    size_t size,
    TypeReflection::ScalarType scalar_type,
    int rows,
...
void BufferCursor::_get_matrix(void* data, size_t size, TypeReflection::ScalarType scalar_type, int rows, int cols)
    const
{
    check_matrix(size, scalar_type, rows, cols);
    if (rows > 1) {
...
#define GETSET_SCALAR(type, scalar_type)                                                                               \
    template<>                                                                                                         \
    SGL_API void BufferCursor::set(const type& value) const                                                            \
    {                                                                                                                  \
        _set_scalar(&value, sizeof(value), TypeReflection::ScalarType::scalar_type);                                   \
    }                                                                                                                  \
    template<>                                                                                                         \
    SGL_API void BufferCursor::get(type& value) const                                                                  \
    {                                                                                                                  \
...
template<>
SGL_API void BufferCursor::set(const bool& value) const
{
    uint v = value ? 1 : 0;
    _set_scalar(&v, sizeof(v), TypeReflection::ScalarType::bool_);
}
template<>
SGL_API void BufferCursor::get(bool& value) const
{
    uint v;
...
template<>
SGL_API void BufferCursor::set(const bool2& value) const
{
    uint2 v = {value.x ? 1 : 0, value.y ? 1 : 0};
    _set_vector(&v, sizeof(v), TypeReflection::ScalarType::bool_, 2);
}
template<>
SGL_API void BufferCursor::get(bool2& value) const
{
    uint2 v;
...
template<>
SGL_API void BufferCursor::set(const bool3& value) const
{
    uint3 v = {value.x ? 1 : 0, value.y ? 1 : 0, value.z ? 1 : 0};
    _set_vector(&v, sizeof(v), TypeReflection::ScalarType::bool_, 3);
}
template<>
SGL_API void BufferCursor::get(bool3& value) const
{
    uint3 v;
...
void BufferCursor::write_data(size_t offset, const void* data, size_t size) const
{
